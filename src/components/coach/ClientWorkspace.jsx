...
const tabs = ['Summary', 'Journal', 'Notes', 'Tasks']
const sectionMeta = {
...
export default function ClientWorkspace({ client, initialSection = 'Summary', mode = 'full' }) {
  ...
  const recordCard = e => <article key={e.id} data-coach-record={e.kind} className={`cw-panel cw-record ${e.details.pinned ? 'cw-record-pinned' : ''}`}>
    ...
    {e.kind === 'task' && <div className="cw-task-meta"><span className={`cw-pill ${e.details.state !== 'done' && e.details.due && e.details.due < day() ? 'cw-overdue' : ''}`}>{e.details.state === 'done' ? 'Completed' : e.details.due && e.details.due < day() ? 'Overdue' : 'Open'}</span><span className="cw-pill">{e.details.priority || 'normal'} priority</span><span className="cw-muted">{(e.details.owner || 'Coach').replaceAll(` ${String.fromCharCode(8212)} `, ', ')} · {e.details.due ? `Due ${workspaceDate(e.details.due)}` : 'No due date'}</span></div>}
