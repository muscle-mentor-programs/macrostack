...
export default function CustomerStoreLinks() {
  ...
  return <section className="app-page-inset mb-6 glass-card border border-border rounded-2xl p-4 anim-fade-in-up" aria-label="Linked stores">
    <div className="flex items-center gap-2 mb-3"><Store size={14} className="text-olive-light" /><h2 className="profile-card-heading">{connections?.length > 1 ? 'LINKED STORES' : 'LINKED STORE'}</h2></div>
    {error ? <div><p role="alert" className="text-sm text-muted">{error}</p><button type="button" className="profile-card-action underline py-3" onClick={() => setRevision(value => value + 1)}>Retry store connection</button></div> : !connections ? <p role="status" className="text-xs text-muted">Checking store connection…</p> : <div className="space-y-4">
      {connections.map(connection => <div key={connection.id} className="space-y-3">
        ...
        <button type="button" className="profile-card-action flex items-center gap-2 min-h-11 px-3 py-2 rounded-xl border border-border bg-surface hover:border-brown/50 transition-colors" onClick={() => openChat(connection.id)}><MessageCircle size={14} aria-hidden="true" />Message store</button>
      </div>)}
    </div>}
  </section>
}
