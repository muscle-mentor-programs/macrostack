...
export default function Team({ organizationId, location, locations, operators = [], corporate, onChanged }) {
  ...
  return <section className="retail-section retail-team retail-setup-target retail-store-scroll-target" id="retail-team" tabIndex={-1} aria-label="Team & permissions">
    <div className="team-heading"><div><span className="retail-eyebrow">PEOPLE & ACCESS</span><h2 className="retail-store-section-heading">Team & permissions</h2><p>Give each employee the tools and access they need.</p></div><Button primary onClick={() => setEditing({})}><UserPlus size={16} />Invite employee</Button></div>
...
function EmployeeEditor({ initial, members, corporate, organizationId, location, locations, operators, onClose, onSaved }) {
  ...
  const effectivePermissions = role === 'organization_admin' ? permissions : Object.fromEntries(Object.entries(permissions).filter(([key]) => key !== 'branding'));
  const payload = { organization_id: organizationId, id: initial.id, revision: initial.revision, access_role: role, permissions: effectivePermissions, location_id: currentStore, location_ids: locationIds, operator_id: operatorId, email, reassign_to: replacement || null, confirm_unassigned: unassigned };
  ...
  return <Modal wide title={initial.id ? 'Employee access' : 'Invite employee'} onClose={onClose}><div className="team-editor">
    ...
    {suspending ? <>
      ...
      {Object.entries(effectivePermissions).map(([key, value]) => <p key={key}>{capabilityLabels[key]}: {value ? 'Allowed' : 'Not allowed'}</p>)}
