...
export default function CoachResources({ account = false }) {
  ...
  return <section className="coach-resource-page"><header><p className="coach-eyebrow">COACH PORTAL</p><h1>{account?'Account & tools':'Library'}</h1><p>{account?'Your settings and supporting tools.':'Everything you use to support your clients.'}</p></header><div className="coach-resource-grid">{items.map(([id,title,description,Icon])=><button key={id} onClick={()=>setActivePage(id)}><Icon size={22}/><span><strong>{title}</strong><small>{description}</small></span><ArrowRight size={18}/></button>)}</div>{account&&<section className="coach-account-settings"><h2>Preferences</h2><div><span>Appearance</span><ThemeToggle /></div><button onClick={()=>setActiveRole(null)}>Switch to member app</button>{currentUser?.role==='superadmin'&&<button onClick={()=>setPortalMode(portalMode==='superadmin'?'coach':'superadmin')}>Switch to {portalMode==='superadmin'?'coach':'superadmin'} portal</button>}<button onClick={logout}>Log out</button></section>}</section>
}
