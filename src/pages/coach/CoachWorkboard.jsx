...
export default function CoachWorkboard({ renderClient }) {
  ...
  const rows = clients.filter(c => c.status !== 'archived').map(client => ({ client,
  ...
  return <div className="dashboard-workboard">
    ...
    <div className="dashboard-client-grid">{visibleRows.map((r, i) => renderClient(r.client, i, () => open(r.client)))}</div>
    ...
    {!clients.length&&<div className="cw-panel"><h3>Your client workspace starts here</h3><p>Add your first client, then manage their check-ins, notes, and next steps in one place.</p><button onClick={()=>setActivePage('clients')}>Add a client</button></div>}
    <section className="cw-panel"><h3>Upcoming follow-ups</h3>{tasks.slice().sort((a,b)=>(a.details.due||'9999').localeCompare(b.details.due||'9999')).slice(0,12).map(t=>{const client=clients.find(c=>c.id===t.client_id);return <div className="cw-row" key={t.id}><span>{t.details.due ? format(new Date(t.details.due + 'T12:00:00'), 'MMMM d, yyyy') : 'No date'} · {client?.name || 'Client'} · {t.title}</span>{client&&<button onClick={()=>open(client)}>Review</button>}</div>})}{!tasks.length&&!recordsUnavailable&&<p className="cw-muted">No open follow-ups.</p>}</section>
  </div>
}
