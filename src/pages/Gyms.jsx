...
export default function Gyms() {
  ...
  useEffect(() => {
    ...
    const followHash = () => { if (window.location.hash) scrollToMarketingSection(window.location.hash.slice(1)) }
    const hashTimer = setTimeout(followHash, 150)
    window.addEventListener('hashchange', followHash)
    const title = document.title
    document.title = 'Macrostack for Gyms | Pro for Members. Tools for Trainers.'
    return () => { clearTimeout(hashTimer); window.removeEventListener('hashchange', followHash); document.title = title; document.documentElement.classList.remove('landing-mode') }
  }, [])
  ...
  return <div ref={rootRef} className="gyms-page" onClick={event => {
    const link = event.target.closest('a[href^="#"]')
    if (!event.defaultPrevented && link && scrollToMarketingSection(link.hash.slice(1))) event.preventDefault()
  }}>
