...
serve(async req=>{
  ...
  try {
    ...
    if(action==='manage') {
      ...
      const {data:order,error:orderError}=await db.from('marketplace_orders').select('*').eq('id',access.order_id).eq('buyer_id',user.id).single()
      ...
      const portal=await stripe.billingPortal.sessions.create({customer:order.customer_id,return_url:`${base}/marketplace`},orderOptions(order))
      return json({url:portal.url})
    }
    if(action==='status') {
      if(input.session_id) {
        const lookup=await db.from('marketplace_orders').select('*').eq('session_id',input.session_id).eq('buyer_id',user.id).single()
        if(lookup.error || !lookup.data) throw new Error('This checkout does not belong to your account.')
        const session=await stripe.checkout.sessions.retrieve(input.session_id,{},orderOptions(lookup.data))
        ...
        await fulfillMarketplace(stripe,db,session,orderOptions(lookup.data).stripeAccount)
      }
      ...
      const {data:order}=await db.from('marketplace_orders').select('*').eq('id',access.order_id).single()
      if(order?.subscription_id) await syncMarketplaceSubscription(stripe,db,order.subscription_id,orderOptions(order).stripeAccount)
    ...
    if(action==='checkout') {
      ...
      if(access) {
        const {data:previous}=await db.from('marketplace_orders').select('*').eq('id',access.order_id).single()
        if(previous?.subscription_id) {
          const sub=await stripe.subscriptions.retrieve(previous.subscription_id,{expand:['latest_invoice']},orderOptions(previous))
      ...
      const connection=await coachConnection(db,listing.coach_id)
      if(!connection || !coach?.coach_code) throw new Error('This coach needs to connect their own Stripe account before accepting new payments.')
      if(!Deno.env.get('STRIPE_CONNECT_WEBHOOK_SECRET')) throw new Error('Direct coach payments are still being configured. Please try later.')
      ...
      if(order?.session_id) {
        const existing=await stripe.checkout.sessions.retrieve(order.session_id,{},orderOptions(order))
      ...
      if(!order) {
        const created=await db.from('marketplace_orders').insert({client_id:client.id,buyer_id:user.id,coach_id:listing.coach_id,
          price_cents:listing.price_cents,billing_mode:listing.billing_mode,duration_days:listing.duration_days,destination:connection.stripe_account_id,payment_flow:'direct'}).select().single()
      ...
      const session=await stripe.checkout.sessions.create({mode:order.billing_mode==='monthly'?'subscription':'payment',
        ...
        ...(order.billing_mode==='monthly'?{subscription_data:{metadata,...(order.payment_flow==='direct'?{}:{transfer_data:{destination:order.destination}})}}:{payment_intent_data:{metadata,...(order.payment_flow==='direct'?{}:{transfer_data:{destination:order.destination}})}}),
      ...
      },{...orderOptions(order),idempotencyKey:`marketplace-${order.id}`})
