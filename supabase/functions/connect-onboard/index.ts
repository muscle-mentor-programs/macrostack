...
serve(async (req) => {
  ...
  try {
    ...
    const stripe = new Stripe(key, { apiVersion: '2024-06-20', httpClient: Stripe.createFetchHttpClient(), maxNetworkRetries: 0, timeout: 20000 })
    const input = await req.json()
    return json(await withStripeOperationLock(db, user.id, () => connectCoach({ db, stripe, userId: user.id,
