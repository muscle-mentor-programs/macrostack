...
try {
  for (const width of [390, 1440]) {
    ...
    await page
      ...
      .click();
    const [planPDF] = await Promise.all([page.waitForEvent('download',{timeout:15000}), page.getByTitle('Download as PDF').click()]);
    assert.equal(planPDF.suggestedFilename(), 'retail-weekly-plan.pdf');
    await page
