...
h=harness({user:{...retailer,email:'demo@getmacrostack.com'},invite:{id:'demo-invite',email,token:'token',role:'customer',store_name:'MacroStack Retail'}});assert.equal((await h.call({action:'invite',invitation_id:'demo-invite'})).status,403);assert.equal(h.sent.length,0);
