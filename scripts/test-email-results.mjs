...
for (const endpoint of ['notify', 'report', 'meal-plan', 'send']) {
  ...
  for (const rejected of [true, false]) {
    ...
    const context = vm.createContext({ Resend, requireEmailRecipients: async () => true, requireUser: async () => ({ user: { id: 'fixture' } }), process: { env: { RESEND_API_KEY: 'fixture' } }, newMessageTemplate: () => '', welcomeTemplate: () => '', reminderTemplate: () => '', mealPlanTemplate: () => '', weeklyReportTemplate: () => '', coachBroadcastTemplate: () => '' })
