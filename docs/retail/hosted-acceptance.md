...
This opt-in test uses real Auth, PostgREST, Edge Functions and private Storage. It is deliberately excluded from CI. It creates four synthetic accounts and one synthetic store, uses no actual customer data, opts out of external reminders, and never invokes checkout or a messaging provider.

## Run safely

1. Create a private temporary JSON file outside the repository, with permissions `0600`, containing `url` (the MacroStack Supabase URL) and `key` (its public anon key). Do not use a service-role key. The test writes generated account credentials here; never print, commit or share this file.
2. Run `node scripts/test-retail-live.mjs register /absolute/private/state.json`. This uses the existing registration endpoint and real password sign-in. The endpoint does not send confirmation mail. Review any changes to that endpoint before running this test again.
